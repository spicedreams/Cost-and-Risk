...
def estimator_group(prefix: str, estimate_type: str, count: int,
    ...
    for index in range(1, count + 1):
        ...
        if probability is not None:
            ...
            prob = float(rng.randint(1, 100)) / 100.0
        else:
            prob = 1.0
...
def build_fixture_trees(master_seed: int) -> List[Node]:
    ...
    def event_cost_estimators(prefix: str, count: int, event_type: str) -> List[Node]:
        """Cost estimates gated by each estimator's event probability."""
        result: List[Node] = []
        for index in range(1, count + 1):
            opt, likely, pess = cost_values(rng)
            probability = 1.0 if event_type == "Issue" else float(rng.randint(1, 100)) / 100.0
            result.append(make_estimator(
                f"{prefix}-E{index}", f"Estimator {index}", "Cost",
                opt, likely, pess, probability
            ))
        return result

    def residual_estimators(prefix: str, pattern: Sequence[str]) -> List[Node]:
        """Individual residual estimates; Risk and Issue opinions may be mixed."""
        result: List[Node] = []
        for index, residual_type in enumerate(pattern, start=1):
            opt, likely, pess = cost_values(rng)
            probability = 1.0 if residual_type == "Issue" else float(rng.randint(1, 100)) / 100.0
            result.append(make_estimator(
                f"{prefix}-E{index}", f"Residual estimator {index}", residual_type,
                opt, likely, pess, probability
            ))
        return result

    def treatment(prefix: str, name: str, residual_pattern: Sequence[str]) -> Node:
        treatment_cost = Node(
            f"{prefix}-COST", "Treatment implementation cost", "Aggregate", "Cost",
            children=estimator_group(f"{prefix}-COST", "Cost", 2, rng, probability=1.0)
        )
        residual = Node(
            f"{prefix}-RES", "Residual exposure", "Aggregate", "Residual",
            children=residual_estimators(f"{prefix}-RES", residual_pattern)
        )
        return Node(prefix, name, "Aggregate", "Treatment",
                    children=[treatment_cost, residual])

    def terminal_event(prefix: str, name: str, event_type: str,
                       treatment_patterns: Sequence[Sequence[str]]) -> Node:
        untreated_cost = Node(
            f"{prefix}-COST", "Untreated event cost", "Aggregate", "Cost",
            children=event_cost_estimators(f"{prefix}-COST", 3, event_type)
        )
        treatments = [
            treatment(f"{prefix}-TR{index}", f"Treatment alternative {index}", pattern)
            for index, pattern in enumerate(treatment_patterns, start=1)
        ]
        return Node(prefix, name, "Aggregate", event_type,
                    children=[untreated_cost, *treatments])

    def consensus_event(prefix: str, name: str, event_type: str, count: int = 3) -> Node:
        probability = 1.0 if event_type == "Issue" else None
        return Node(prefix, name, "Aggregate", event_type,
                    children=estimator_group(prefix, event_type, count, rng, probability))

    # Risks: level 1 has Aggregate Risk children only. Below level 1, each
    # Risk Aggregate has either homogeneous Risk children or one Cost child
    # plus one or more mutually exclusive Treatment children.
    risk_depth_4 = consensus_event("R-D4", "Supplier insolvency", "Risk")
    risk_depth_3a = Node("R-D3A", "Third-party risks", "Aggregate", "Risk",
                         children=[risk_depth_4])
    risk_depth_3b = terminal_event(
        "R-D3B", "Data migration failure", "Risk",
        [("Risk", "Risk", "Issue"), ("Risk", "Risk")]
    )
    risk_depth_2a = Node("R-D2A", "Delivery risks", "Aggregate", "Risk",
                         children=[risk_depth_3a, risk_depth_3b])

    risk_depth_6 = consensus_event("R-D6", "Specialist availability", "Risk")
    risk_depth_5 = Node("R-D5", "Resource risks", "Aggregate", "Risk",
                        children=[risk_depth_6])
    risk_depth_4b = Node("R-D4B", "Capability risks", "Aggregate", "Risk",
                         children=[risk_depth_5])
    risk_depth_3c = Node("R-D3C", "People risks", "Aggregate", "Risk",
                         children=[risk_depth_4b])
    risk_depth_2b = Node("R-D2B", "Organisational risks", "Aggregate", "Risk",
                         children=[risk_depth_3c])

    risk_depth_2c = terminal_event(
        "R-D2C", "Cybersecurity incident", "Risk",
        [("Risk", "Risk"), ("Issue", "Issue")]
    )
    risk_root = Node("R-ROOT", "Risks", "Root", "Risk",
                     children=[risk_depth_2a, risk_depth_2b, risk_depth_2c])

    # Issues mirror Risks, but ordinary Issue estimators and untreated Issue
    # costs always use probability 1.00. Residual opinions may still mix Risk
    # and Issue estimates because estimators can disagree about certainty.
    issue_depth_4 = consensus_event("I-D4", "Confirmed integration defect", "Issue")
    issue_depth_3a = Node("I-D3A", "Technical issues", "Aggregate", "Issue",
                          children=[issue_depth_4])
    issue_depth_3b = terminal_event(
        "I-D3B", "Current supplier breach", "Issue",
        [("Issue", "Issue", "Risk"), ("Issue", "Issue")]
    )
    issue_depth_2a = Node("I-D2A", "Delivery issues", "Aggregate", "Issue",
                          children=[issue_depth_3a, issue_depth_3b])

    issue_depth_6 = consensus_event("I-D6", "Unresolved data defect", "Issue")
    issue_depth_5 = Node("I-D5", "Data quality issues", "Aggregate", "Issue",
                         children=[issue_depth_6])
    issue_depth_4b = Node("I-D4B", "Migration issues", "Aggregate", "Issue",
                          children=[issue_depth_5])
    issue_depth_3c = Node("I-D3C", "Platform issues", "Aggregate", "Issue",
                          children=[issue_depth_4b])
    issue_depth_2b = Node("I-D2B", "Operational issues", "Aggregate", "Issue",
                          children=[issue_depth_3c])

    issue_depth_2c = terminal_event(
        "I-D2C", "Current service interruption", "Issue",
        [("Issue", "Issue"), ("Risk", "Issue")]
    )
    issue_root = Node("I-ROOT", "Issues", "Root", "Issue",
                      children=[issue_depth_2a, issue_depth_2b, issue_depth_2c])
...
def validate_tree_structure(roots: Sequence[Node]) -> None:
    ...
    for node, depth in preorder(roots):
        ...
        if node.estimate_type == "Treatment" and node.node_type in PARENT_NODE_TYPES:
            cost_children = [child for child in node.children if child.estimate_type == "Cost"]
            residual_children = [child for child in node.children if child.estimate_type == "Residual"]
            if len(node.children) != 2 or len(cost_children) != 1 or len(residual_children) != 1:
                raise ValueError(
                    f"Treatment {node.node_id} must have exactly one Cost and one Residual child")

    def require_estimator_children(parent: Node, allowed_types: set[str]) -> None:
        if not parent.children or any(child.node_type != "Estimator" for child in parent.children):
            raise ValueError(f"{parent.node_id} must have Estimator children only")
        if any(child.estimate_type not in allowed_types for child in parent.children):
            raise ValueError(
                f"{parent.node_id} has an Estimator outside allowed types {sorted(allowed_types)}")

    def validate_treatment_node(parent: Node) -> None:
        cost = next(child for child in parent.children if child.estimate_type == "Cost")
        residual = next(child for child in parent.children if child.estimate_type == "Residual")
        if cost.node_type != "Aggregate" or residual.node_type != "Aggregate":
            raise ValueError(f"Treatment {parent.node_id} children must both be Aggregates")
        require_estimator_children(cost, {"Cost"})
        if any(float(child.prob) != 1.0 for child in cost.children):
            raise ValueError(f"Treatment cost {cost.node_id} must be unconditional")
        require_estimator_children(residual, {"Risk", "Issue"})
        for child in residual.children:
            probability = float(child.prob)
            if child.estimate_type == "Issue" and probability != 1.0:
                raise ValueError(f"Residual Issue {child.node_id} must have probability 1.00")
            if child.estimate_type == "Risk" and not 0.01 <= probability <= 0.99:
                raise ValueError(f"Residual Risk {child.node_id} probability must be 0.01..0.99")

    def validate_event_node(parent: Node) -> None:
        event_type = parent.estimate_type
        child_kinds = {child.node_type for child in parent.children}
        if child_kinds == {"Estimator"}:
            require_estimator_children(parent, {event_type})
            for child in parent.children:
                probability = float(child.prob)
                if event_type == "Issue" and probability != 1.0:
                    raise ValueError(f"Issue estimator {child.node_id} must have probability 1.00")
                if event_type == "Risk" and not 0.01 <= probability <= 0.99:
                    raise ValueError(f"Risk estimator {child.node_id} probability must be 0.01..0.99")
            return
        if child_kinds != {"Aggregate"}:
            raise ValueError(
                f"{parent.node_id} may not mix Aggregate and Estimator children")

        child_types = [child.estimate_type for child in parent.children]
        if all(child_type == event_type for child_type in child_types):
            for child in parent.children:
                validate_event_node(child)
            return

        cost_children = [child for child in parent.children if child.estimate_type == "Cost"]
        treatment_children = [child for child in parent.children if child.estimate_type == "Treatment"]
        if (len(cost_children) != 1 or not treatment_children or
                len(cost_children) + len(treatment_children) != len(parent.children)):
            raise ValueError(
                f"{parent.node_id} needs either same-type event children or one Cost plus Treatments")
        untreated_cost = cost_children[0]
        require_estimator_children(untreated_cost, {"Cost"})
        for child in untreated_cost.children:
            probability = float(child.prob)
            if event_type == "Issue" and probability != 1.0:
                raise ValueError(f"Issue cost estimate {child.node_id} must have probability 1.00")
            if event_type == "Risk" and not 0.01 <= probability <= 0.99:
                raise ValueError(f"Risk cost estimate {child.node_id} probability must be 0.01..0.99")
        for child in treatment_children:
            validate_treatment_node(child)

    risk_root = next(root for root in roots if root.estimate_type == "Risk")
    issue_root = next(root for root in roots if root.estimate_type == "Issue")
    for root, event_type in ((risk_root, "Risk"), (issue_root, "Issue")):
        if any(child.node_type != "Aggregate" or child.estimate_type != event_type
               for child in root.children):
            raise ValueError(
                f"Level-1 {event_type} root may have Aggregate {event_type} children only")
        for child in root.children:
            validate_event_node(child)
